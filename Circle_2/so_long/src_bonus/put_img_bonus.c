...
/*   Updated: 2022/12/26 13:49:22 by ejachoi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_game_scene(t_game *game)
{
	...
	if (game->flag.scene == GAME_STARTING)
		mlx_put_image_to_window \
			(game->mlx_ptr, game->win_ptr, game->object.opening.ptr, 0, 0);
...
void	put_step_count(t_game *game)
{
	...
	count = game->object.player.step.count;
	str = ft_itoa(count);
...
void	put_kill(t_game *game)
{
	if (game->flag.kill)
		mlx_put_image_to_window \
			(game->mlx_ptr, game->win_ptr, game->object.kill.ptr, 0, 0);
}
